...
public record UtenteDTO(
        ...
        @Pattern(regexp = "^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=\\S+$){2,4}$", message = "Formato password non valido")
        String email,
        @NotBlank(message = "Il campo \"password\" è obbligatorio")
        @Pattern(regexp = "^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{4,}$",
                message = "La password deve contenere una maiuscola, una minuscola, un numero e deve essere lunga almeno 8 caratteri.")
        String password,
        @NotBlank(message = "Il campo \"ruolo utente\" è obbligatorio")
